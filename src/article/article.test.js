...
describe('GET /articles', () => {
    ...
    it('should not found article', async () => {
        const res = await request(app).get('/articles/1000');
...
describe('PUT /articles/:id', () => {
    it('should update article successfully', async () => {
        const res = await request(app).put('/articles/5')
            .send({
                Title: "Title_" + datetime,
                Summary: "Summary",
                Content: "Content",
    ...
    it('should return that Title existed', async () => {
        const res = await request(app).put('/articles/6')
            .send({
                Title: "Title_1",
                Summary: "Summary",
                Content: "Content",
    ...
    it('should return that not Article of User', async () => {
        const res = await request(app).put('/articles/1')
            .send({
                Title: "Title_134",
                Summary: "Summary",
                Content: "Content",
        ...
        expect(res.status).toEqual(500);
        expect(res.body.message).toEqual('Cannot update Article of another User');
    });
});

describe('DELETE /articles/:id', () => {
    it('should delete article successfully', async () => {
        const res = await request(app).delete('/articles/7')
            .send({
                UserId: 22
            });
    ...
    it('should return cannot delete article of another user', async () => {
        const res = await request(app).delete('/articles/6')
            .send({
                UserId: 1
            })

        expect(res.status).toEqual(500);
        expect(res.body.message).toEqual("Cannot delete Article of another User");
    })
});

describe('POST /articles/getAllArticleOfUser', () => {
    it('should return array of articles belong user', async () => {
        const res = await request(app).post('/articles/getAllArticleOfUser')
            .send({
                UserId: 22
            });
        expect(res.status).toEqual(200);
