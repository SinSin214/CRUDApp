...
describe("PUT /articles/:id", () => {
    ...
    it("should return that Title existed", async () => {
        const res = await request(app).put("/articles/6").send({
            Title: "Title_1",
            Summary: "Summary",
            Content: "Content",
            UserId: 1,
    ...
    it("should return that not Article of User", async () => {
        const res = await request(app).put("/articles/1").send({
            Title: "Title_134",
            Summary: "Summary",
            Content: "Content",
            UserId: 22,
...
describe("DELETE /articles/:id", () => {
    it("should delete article successfully", async () => {
        const res = await request(app).delete("/articles/7").send({
            UserId: 22,
        });
    ...
    it("should not found article", async () => {
        const res = await request(app).delete("/articles/1000").send({
            UserId: 10,
        });
    ...
    it("should return cannot delete article of another user", async () => {
        const res = await request(app).delete("/articles/6").send({
            UserId: 1,
        });
